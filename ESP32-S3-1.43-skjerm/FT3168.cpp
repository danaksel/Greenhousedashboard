...
static const TickType_t TOUCH_I2C_TIMEOUT_TICKS = pdMS_TO_TICKS(30);
static volatile uint32_t touch_read_count = 0;
static volatile uint32_t touch_press_count = 0;
static volatile uint32_t touch_error_count = 0;
static volatile esp_err_t touch_last_error = ESP_OK;
...
static esp_err_t I2C_writr_buff(uint8_t addr,uint8_t reg,uint8_t *buf,uint8_t len)
{
  ...
  esp_err_t ret = i2c_master_write_to_device(TEST_I2C_PORT,addr,pbuf,len+1,TOUCH_I2C_TIMEOUT_TICKS);
  free(pbuf);
  return ret;
}
...
{
  return i2c_master_write_read_device(TEST_I2C_PORT,addr,&reg,1,buf,len,TOUCH_I2C_TIMEOUT_TICKS);
}
...
uint8_t getTouch(uint16_t *x,uint16_t *y)
{
  ...
  {
    touch_error_count++;
    touch_last_error = ret;
    return 0;
  }
  touch_last_error = ESP_OK;
